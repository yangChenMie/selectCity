var citySelect=function (option) {
    ...
    this.flag=true;
    this.hotcity=["北京-PEK-1-3526-2","上海-SHA-860-3527-2","天津-TSN-20-3529-2","重庆-CKG-2469-3528-2","厦门-XMN-1280-1264-2","成都-CTU -2511-2510-2","深圳-SZX-2167-2140-2","深圳-SZX-2167-2140-2","济南-TNA-1492-1491-2"];
    ...
    console.log(this.cityObjL);
}
citySelect.prototype={
    init:function () {
        if($(".citySelectDom"+this.option.type).length>0){
            this._slider(".citySelectDom"+this.option.type);
        }else {
            $("body").append("<div class='citySelectDom citySelectDom"+this.option.type+"' style='position: absolute;top: 0;left: 0;right: 0;bottom: 0; z-index: 999;background: #eeeeee'></div>");
            var ohtml = '<div id="showLetter" class="showLetter"><span>A</span></div>' +
                '<div class="allContent disFlex " data-id="" cityNa="">\n' +
                '        <div class="cityMain flex1">\n' +
                '            <div class="searchCity">\n' +
                '                <input class="serchInput" type="text" placeholder="北京/PEK/beijing/bj">\n' +
                '            </div>\n' +
            ...
            $(".citySelectDom"+this.option.type).append(ohtml);
            this.addCityData();
        }
        this.staticFn();
        this.loactionCity();
    ...
    addCityData:function () {
        ...
        for(var i=0;i<9;i++){
            ...
            if(this.option.type==0){
                //机票
                $(".hotCityWrap").append('<p class="hotCity smallP city" data-id="'+this.hotcity[i].split("-")[1]+'">'+city+'</p>');
            }else{
                //酒店
                $(".hotCityWrap").append('<p class="hotCity smallP city" data-id="'+this.hotcity[i].split("-")[2]+'">'+city+'</p>');
            }
        }
        /*拼音首字母填充*/
        for(var j=0;j<this.letters.length;j++){
            var letter=this.letters[j];
            $(".letter").append('<li><a href="javascript:;">'+letter+'</a></li>');
        }
        /*城市列表填充*/
        for(var j=0;j<this.letters.length;j++){
            var letter=this.letters[j];
            var ohtml='<div class="oneCityList">\n';
                for(key in this.cityObjL){
                    ohtml+='<div class="listTitle" id="'+key+'1">'+key+'</div>';
    ...
    _callback: function() {
        var me = this;
        var cityNameme=$(".allContent").attr("cityNa");
        var dataId=$(".allContent").attr("data-id");
    ...
    staticFn:function () {
        ...
        $('body').on('click', '.letter a', function () {
            var s = $(this).html();
            $(window).scrollTop($('#' + s + '1').offset().top-$(".searchCity").offset().top-$(".hotCityWrap").offset().top);
            $("#showLetter span").html(s);
            $(".showLetter").show().delay(500).hide(0);
        });
        var letterHei=[];
        $.each($(".letter a"),function (i,e) {
        ...
        $(".letter").on('touchstart',function(e) {
            ...
            var x = pos.x, y = pos.y;
            var width = $(".letter").find("a").width();
            var height = $(".letter").find("a").height();
        ...
        $(".city").on("click",function (e) {
            ...
            if(me.flag){
                $(".allContent").attr("cityNa",cityName);
                $(".allContent").attr("data-id",dataId);
                $(".acitveCity").removeClass("acitveCity");
                $(this).addClass("acitveCity")
                $(".citySelectDom"+me.option.type).hide();
                $(window).scrollTop(0);
                me._callback();
            }
            me.flag=false;
            ...
            if($(this).val()==""){
                ...
                $.ajax({
                    ...
                    success:function (res) {
                        searchRsl=res.split(",");
                        if(searchRsl.length>0){
                            var searchData=' <div class="listTitle" id=" ">搜索结果</div>';
                            $.each(searchRsl,function (i,e) {
                                if(me.option.type==0) {
                                    //机票
                                    searchData += '<p class="oneCity city" data-id="'+e.split("|")[3]+' ">' + e.split("|")[0] + ',' + e.split("|")[1] + ',' + e.split("|")[2] + ',' + e.split("|")[3] + '</p>';
                                }else{
                                    //酒店
                                    searchData += '<p class="oneCity city" data-id="'+e.split("|")[4]+'">' + e.split("|")[0] + ',' + e.split("|")[1] + ',' + e.split("|")[2] + ',' + e.split("|")[3] + '</p>';
                                }
                            })
                        }else{
            ...
            if($(this).text()=="请选择"){
                ...
                $.each(me.ocityList,function (i,e) {
                    if(e.indexOf(locity.substring(0,locity.length-1))>-1){
                        if(me.option.type==0){
                            $(".citySelectDom"+me.option.type).find(".mapCity").attr("data-id",e.split("|")[3]);
                        }else{
                            $(".citySelectDom"+me.option.type).find(".mapCity").attr("data-id",e.split("|")[4]);
                        }
                        return false;
                    }
                })
    ...
    loactionCity:function () {
        ...
        function myFun(result){
            ...
            $(".citySelectDom"+me.option.type).find(".mapCity").text(cityName);
        }
